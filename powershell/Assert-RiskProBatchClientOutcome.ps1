function Assert-RiskProBatchClientOutcome {
  ...
  Process {
    ...
    if ($Quiet) {
      ...
      # Parse outcome
      if ($Success -eq $false) {
        if (Select-String -InputObject $Log -Pattern "COMPLETED_WITH_ERRORS" -SimpleMatch -Quiet) {
          Write-Log -Type "WARN" -Object "$Object completed with errors"
          $Global:ErrorCount++
        } elseif (Select-String -InputObject $Log -Pattern "FAILED" -SimpleMatch -Quiet) {
          Write-Log -Type "ERROR" -Object "$Object failed"
          $Global:ErrorCount++
        } else {
          # If operation failed
